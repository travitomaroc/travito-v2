...
export const callClaude = async (system, user, history=[], attempt=1) => {
  ...
  // Auto-retry on overload (529) — up to 3 attempts
  if (attempt < 3 && (res.status === 529 || (d.error?.type === "overloaded_error") ||
      (d.error?.message||"").toLowerCase().includes("overload"))) {
    await new Promise(r => setTimeout(r, attempt * 12000));
    return callClaude(system, user, history, attempt + 1);
  }
